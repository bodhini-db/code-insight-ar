  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1 space-y-5 px-5 py-5 fade-in">
        ...
        <section>
          <h2 className="mb-2 text-base font-bold text-foreground">Detailed Explanation</h2>
            ...
            {explanation.logicFlow.map((step) => (
              <div key={step.step} className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-0.5 text-sm text-muted-foreground">→ {step.explanation}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Code Structure */}
        <section>
          <h2 className="mb-2 text-base font-bold text-foreground">Code Structure</h2>
          <ul className="space-y-1.5">
            {(explanation.codeStructure ?? []).map((s, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-muted-foreground">
                <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-primary" />
                {s}
              </li>
